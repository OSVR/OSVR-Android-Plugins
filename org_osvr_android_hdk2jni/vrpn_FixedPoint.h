...
// This header is slightly modified from the VRPN header of the same name -
// here I'm using standard lib integer and float definitions instead of
// the vrpn typedefs
...
namespace vrpn {
    ...
    private:
        ...
        float get(TypeWrapper<float> *) const
        {
            return static_cast<float>(value_) / (1 << FRACTIONAL_BITS);
        }

        double get(TypeWrapper<double> *) const
        {
